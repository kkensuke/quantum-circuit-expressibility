...
class GenerateCircuit_pennylane:
    ...
    def draw_circuit(self, nqubits, nlayers):
        self.nqubits = nqubits
        self.nlayers = nlayers
...
class Expressibility1norm_pennylane(GenerateCircuit_pennylane):
    ...
    def generate_circuit_density_matrix(self, params):
        dev = qml.device("default.qubit", wires=self.nqubits)

        @qml.qnode(dev)
        def circuit_density_matrix(params):
            self.generate_circuit(params)
